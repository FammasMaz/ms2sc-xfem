...
% Charge le maillage
omega = Mesh(geo2msh('./meshes/plate.geo',1,0.5));
%keyboard
...
upOmega = omega.restrict(@(x) x(:,2) >= H/2);
downOmega = omega.restrict(@(x) x(:,2) <= H/2);


% Ecriture du systeme
K = FEMMat(upOmega, B1) + FEMMat(downOmega, B2); 
F = FEMVec(dtop,Fd);
...
[sigma, strain] = stressmod(omega,downOmega, B1,B2,u);
